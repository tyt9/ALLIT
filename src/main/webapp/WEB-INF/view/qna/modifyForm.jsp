...
<div class="container" style="text-align: center">
	...
	<div style="margin: 30px 100px 30px 100px;">
		...
		<form action="modify" method="post" >
			<input type="hidden" name="no" value="${modReq.QNum}">
			<div>
				<label for="firstName" class="form-label">제목</label>
				<input type="text" name="title" class="form-control" placeholder="<c:if test="${errors.title}">제목을 입력하세요.</c:if>" id="firstName" value="${modReq.QTitle}">
			</div>
			<br>
			<div>
				<label for="zip" class="form-label">내용</label>
				<div class="form-group">
				<textarea name="content"  class="form-control h-25" rows="10" cols="30">${modReq.QContent}</textarea>
			</div>
			<hr class="my-4">
				<div class="find-btn">
					<button class="send" style="background-color: #ff914d;" type="submit">게시글 수정</button>
				</div>
		</form>
		</div>
	</div>
