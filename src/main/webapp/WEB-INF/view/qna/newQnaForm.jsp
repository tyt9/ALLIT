	...
	<div style="margin: 30px 100px 30px 100px;">
		...
		<div class="container now" style="float:none; margin: 100 auto;">
			...
			<div>
				...
				<input type="text" name="title" class="form-control" placeholder="<c:if test="${errors.title}">제목을 입력하세요.</c:if>" value="${param.title}">
			</div>
				<br>
				<div>
					<label for="zip" class="form-label">내용</label>
				<textarea name="content" class="form-control h-25" rows="10" cols="30">${param.content}</textarea>
				</div>
				<hr class="my-4">
				<div class="find-btn">
					<button class="send" style="background-color: #ff914d;" type="submit"> 게시글 등록</button>
				</div>
			</form>
	</div>
	</div>
